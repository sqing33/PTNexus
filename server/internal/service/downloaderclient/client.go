...
// Delete 删除下载器中的种子任务，并按 deleteFiles 决定是否删除实际数据文件。
// 参数/返回：hashes 为 infohash 列表，deleteFiles 为 true 时同步删除下载器保存的数据。
// 失败场景：hash 为空、下载器连接失败、认证失败或 RPC/API 返回错误时返回 error。
// 副作用：会向 qBittorrent/Transmission 或盒子代理发起删除请求，可能删除远程服务器上的文件。
func (d Downloader) Delete(hashes []string, deleteFiles bool) error {
	hashes = compactStrings(hashes)
	if len(hashes) == 0 {
		return errors.New("hash 列表为空")
	}
...
func (d Downloader) deleteQBTorrentsByProxy(hashes []string, deleteFiles bool) error {
	...
	if proxyIP == "" {
		return fmt.Errorf("无法解析代理地址: host=%s", d.Host)
	}
	...
	if response.StatusCode < http.StatusOK || response.StatusCode >= http.StatusMultipleChoices {
		return fmt.Errorf("代理删除返回异常: HTTP %d %s", response.StatusCode, strings.TrimSpace(string(body)))
	}

	parsed := proxyDeleteTorrentsResponse{}
	...
	if !parsed.Success {
		...
		for _, result := range parsed.Results {
			errorsText = append(errorsText, result.Errors...)
		}
		if len(errorsText) == 0 {
			errorsText = append(errorsText, "代理删除失败")
		}
		return errors.New(strings.Join(errorsText, "；"))
	}
	return nil
}
