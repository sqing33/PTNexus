...
// ExtractCHDBits 提取彩虹岛站点的详情页参数。
// 参数/返回：输入详情页上下文与运行期依赖，返回统一 SeedData。
// 失败场景：公共提取失败、标签列表抓取失败。
// 副作用：可能会发起列表页网络请求（用于补全标签）。
func ExtractCHDBits(input Input, runtime Runtime) (SeedData, error) {
	...
	if runtime.FetchTagsFromTorrentList != nil {
		...
		if tagErr != nil {
			return data.Normalize(input.FallbackTitle), fmt.Errorf("彩虹岛标签提取失败: %w", tagErr)
		}
		if len(tags) > 0 {
			data.Tags = tags
		}
