...
- 修复：下载器刷新站点识别优先匹配详情页精确域名，避免我堡种子因共享馒头 tracker 被误判为馒头

### v4.0.43（2026.04.30）

- 修复：维护种子获取数据页的源站点筛选与列表展示统一按站点管理配置判断，避免筛选存在源站点后仍出现无可用源站点数据
- 优化：批量获取源站点优先级设置不再硬编码排除我堡/OurBits，已配置为源站点且有 Cookie 时可正常参与获取
