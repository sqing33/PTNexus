...
- 修复：发种日志页的路由参数改为按当前跳转 query 全量覆盖，避免旧筛选条件残留导致日志搜索结果错误
- 修复：一种多站与一站多种在触发入队或发布后，会携带更精简准确的日志跳转参数并及时刷新列表状态
- 调整：一站多种页移除重新拉取入口，避免继续与转种页职责混用
- 调整：维护种子信息页补回源站与种子 ID 的独立加载能力，并将重新拉取数据能力收回维护页统一处理
- 优化：维护入口提升为与一种多站、一站多种并列的顶层导航，便于把维护视图作为独立操作面使用
