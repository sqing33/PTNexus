# PT Nexus 桌面化实施计划（desktop-go）

## 1. 计划摘要

本计划的目标是：

1. 基于已初始化的 Wails 官方骨架（`desktop-go/`），完成 Windows 桌面版接入方案。
2. 严格分两部分实施：
   - 第一部分：仅修改 `desktop-go/`（先做）。
   - 第二部分：修改其他目录（`server-go/`、`webui-go/`、`updater/`，后做，当前不执行）。
...
1. `desktop-go/` 已通过 `wails init` 生成官方骨架，包含：
   - `desktop-go/main.go`
   - `desktop-go/app.go`
   - `desktop-go/wails.json`
   - `desktop-go/frontend/`
   - `desktop-go/build/`
2. 项目现有业务前后端位于：
   - 前端：`webui-go/`
...
## 第一部分：仅 `desktop-go/` 内实施（先做）

说明：以下任务全部限定在 `desktop-go/` 目录，不修改任何其他目录文件。

### 4.1 目标

1. 让 `desktop-go/` 成为可承接后续接入的稳定桌面壳工程。
2. 先把目录内可独立完成的配置、脚本、结构、文档一次性完善。

### 4.2 任务清单（仅 `desktop-go`）

- [x] A1. 清理并标准化 `desktop-go` 工程结构
  - [x] 保留 Wails 官方骨架必要文件。
  - [x] 增补目录：`desktop-go/internal/`、`desktop-go/scripts/`、`desktop-go/docs/`。

- [x] A2. 完善桌面接入文档（仅本目录）
  - [x] 编写 `desktop-go/docs/integration.md`：说明未来如何挂接 `server-go`、`updater`、`webui-go`。
  - [x] 编写 `desktop-go/docs/build.md`：说明开发/打包命令、目录依赖、输入输出。
  - [x] 在 `desktop-go/README.md` 中加入执行顺序与边界说明。

- [x] A3. 预留桌面入口的分层骨架（不接入业务）
  - [x] 在 `desktop-go/internal/desktopapp/` 预留路由分发结构。
  - [x] 在 `desktop-go/main.go` 规划注释和 TODO：`/api`、`/update`、静态资源三路分发。
  - [x] 仅保留空实现或 mock handler，确保不依赖外部目录即可编译。

- [x] A4. 预置构建脚本（仅本目录）
  - [x] 新增 `desktop-go/scripts/dev-desktop.sh`：桌面开发启动流程（占位可执行）。
  - [x] 新增 `desktop-go/scripts/build-windows.ps1`：Windows 打包流程（占位可执行）。
  - [x] 脚本中写明未来会调用 `webui-go` 构建，但当前不改 `webui-go`。

- [x] A5. 预置配置模板（仅本目录）
  - [x] 在 `desktop-go/docs/env.md` 记录未来需要的环境变量：
    - `PTNEXUS_BASE_DIR`
    - `PTNEXUS_DATA_DIR`
    - `PTNEXUS_STATIC_DIR`
    - `VITE_DESKTOP`
...
- [x] A6. 第一部分验收（仅本目录）
  - [x] `desktop-go` 结构清晰、文档齐全。
  - [x] 本目录脚本具备可执行入口（即使为占位流程）。
  - [x] 计划清楚标注第二部分任务，不产生跨目录修改。

### 4.4 本轮补充：桌面壳指向真实前后端（仍仅改 desktop-go）
...
- [x] C2. 后端接入 `../server-go`（进程内）
  - [x] `desktop-go/go.mod` 添加 `replace` 引用 `../server-go`。
  - [x] `/api/*` 转发到 `server-go` Gin Engine（不再使用 mock）。
  - [x] `/update/*` 通过 updater sidecar 代理（兼容现有网关职责）。

- [x] C3. Windows 图标使用 `webui-go/public/favicon.ico`
  - [x] 打包前复制到 `desktop-go/build/windows/icon.ico`。

- [x] C4. NSIS 安装器中文化 + 默认安装目录
  - [x] 安装器语言改为中文。
  - [x] 默认安装目录改为 `D:\\Program Files (x86)\\...`。

- [x] C5. 优化 `desktop-go/.gitignore`
  - [x] 忽略构建产物目录、安装器临时目录、sidecar 二进制目录。
  - [x] 忽略前端生成文件（`dist`、`wailsjs`、`package-lock`、md5）。

- [x] C6. 提供一条命令完成打包
  - [x] 新增 `desktop-go/scripts/package-windows.sh`。
  - [x] 支持执行单命令：`./desktop-go/scripts/package-windows.sh`。

### 4.3 第一部分交付物

1. `desktop-go/Plan.md`（本文件）
2. `desktop-go/docs/*.md`（集成、构建、环境说明）
3. `desktop-go/scripts/*`（开发与打包脚本占位）
4. `desktop-go/internal/*`（桌面分层骨架占位）
...
### 5.4 桌面与业务接入（回到 `desktop-go/`）

- [ ] B10. 真正接入 `server-go` Gin Engine 到 Wails Handler。
- [ ] B11. 真正接入 updater handlers 到 Wails Handler。
- [ ] B12. 前端构建产物改为使用 `webui-go/dist`（替代默认模板前端）。
