...
class HdkylUploader:
    ...
    def _map_parameters(self) -> dict:
        ...
        # 2. 年代映射 (Processing) - 根据站点HTML校对
        year_str = title_params.get("年份", "")
        if year_str and year_str.isdigit():
            year = int(year_str)
            if year == 2025:
                mapped["processing_sel[4]"] = "11"
            elif year == 2024:
                mapped["processing_sel[4]"] = "10"
            elif year == 2023:
                mapped["processing_sel[4]"] = "1"
            elif year == 2022:
                mapped["processing_sel[4]"] = "2"
            elif year == 2021:
                mapped["processing_sel[4]"] = "3"
            elif year == 2020:
                mapped["processing_sel[4]"] = "4"
            elif year == 2019:
                mapped["processing_sel[4]"] = "5"
            elif year == 2018:
                mapped["processing_sel[4]"] = "6"
            elif year == 2017:
                mapped["processing_sel[4]"] = "7"
            elif year == 2016:
                mapped["processing_sel[4]"] = "8"
            elif year < 2016:
                mapped["processing_sel[4]"] = "9"  # Earlier/更早
        ...
        if is_standard_mediainfo and ('blu' in medium_str.lower()
            ...
            mapped["medium_sel[4]"] = "29"  # Encode
        else:
            mapped["medium_sel[4]"] = "34"  # 默认值: Other
    ...
    def _build_title(self) -> str:
        """
        根据 title_components 参数，按照 HDKYL 的规则拼接主标题。
        """
    ...
    def execute_upload(self):
        ...
        执行上传的核心逻辑。
        """
        logger.info("正在为 HDKYL 站点适配上传参数...")
        try:
            ...
            with open(torrent_path, "rb") as torrent_file:
                ...
                logger.info("正在向 HDKYL 站点提交发布请求...")
            ...
            logger.error(f"发布到 HDKYL 站点时发生错误: {e}")
            logger.error(traceback.format_exc())
            return False, f"请求异常: {e}"


def upload(site_info: dict, upload_payload: dict):
    uploader = HdkylUploader(site_info, upload_payload)
