...
class PtloverUploader:
    ...
    def _map_parameters(self) -> dict:
        ...
        mapped["codec_sel[5]"] = "5"  # 默认值: Other
        for key, value in codec_map.items():
            if key.lower() in codec_str.lower():
                mapped["codec_sel[4]"] = value
                mapped["codec_sel[5]"] = value
                break

        # 4. 分辨率映射 (Resolution) - 根据站点HTML校对
        # 站点默认值 'Other': 0
        resolution_map = {
            '8K': '6',
            '4320p': '6',
        ...
        mapped["team_sel[4]"] = team_map.get(release_group_str,
                                             "5")  # 默认值 Other
        mapped["team_sel[5]"] = team_map.get(release_group_str,
                                             "5")  # 默认值 Other

        # 6. 来源映射 (Source) - 根据站点HTML校对
        # 站点默认值 'Other': 99
        source_map = {
            "中国": "1",
            "CN": "1",
        ...
        source_str = origin_str if origin_str else title_params.get(
            "片源平台", "")
        mapped["source_sel[4]"] = "99"  # 默认值: Other
    ...
    def _build_title(self) -> str:
        """
        根据 title_components 参数，按照 Ptlover 的规则拼接主标题。
        """
    ...
    def execute_upload(self):
        ...
        执行上传的核心逻辑。
        """
        logger.info("正在为 Ptlover 站点适配上传参数...")
        try:
            ...
            with open(torrent_path, "rb") as torrent_file:
                ...
                logger.info("正在向 Ptlover 站点提交发布请求...")
            ...
            logger.error(f"发布到 Ptlover 站点时发生错误: {e}")
            logger.error(traceback.format_exc())
            return False, f"请求异常: {e}"


def upload(site_info: dict, upload_payload: dict):
    uploader = PtloverUploader(site_info, upload_payload)
