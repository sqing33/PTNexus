...
def get_data_api():
    ...
    try:
        ...
        site_filter_names = json.loads(request.args.get("siteFilterNames", "[]"))
    ...
    try:
        ...
        for t in torrents_raw:
            ...
            if t.get("sites"):
                agg["sites"][t["sites"]]["uploaded"] = (
                    agg["sites"][t["sites"]].get("uploaded", 0) + upload_for_this_hash
                )
                agg["sites"][t["sites"]]["comment"] = t.get("details")
        ...
        if sort_prop and sort_order:
            ...
            if sort_key in ["size", "progress", "total_uploaded", "site_count"]:
